...
func NewMessage(audioDevices core.AudioDevice, status int, channel, data1, data2 core.Valueable) Message {
	return Message{audioDevices: audioDevices, status: status, channel: channel, data1: data1, data2: data2}
}
...
func (m Message) S() core.Sequence {
	...
	out, err := devices.Output(core.Int(m.deviceID))
	if err != nil {
