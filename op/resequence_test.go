...
func TestResequence_S(t *testing.T) {
	...
	r := NewResequencer([]core.Sequenceable{s}, core.On("1 3 2"))
	if got, want := r.S().Storex(), "sequence('C E D')"; got != want {
		t.Errorf("got [%v:%T] want [%v:%T]", got, got, want, want)
	}
	r = NewResequencer([]core.Sequenceable{s}, core.On(""))
	if got, want := r.S().Storex(), "sequence('C D E')"; got != want {
		t.Errorf("got [%v:%T] want [%v:%T]", got, got, want, want)
	}
	r = NewResequencer([]core.Sequenceable{s}, core.On("1 4 2"))
	if got, want := r.S().Storex(), "sequence('C D')"; got != want {
		t.Errorf("got [%v:%T] want [%v:%T]", got, got, want, want)
	}
	r = NewResequencer([]core.Sequenceable{s}, nil)
	if got, want := r.S().Storex(), "sequence('C D E')"; got != want {
		t.Errorf("got [%v:%T] want [%v:%T]", got, got, want, want)
	}
}
...
func TestResequence_Storex(t *testing.T) {
	...
	// r = NewResequencer([]core.Sequenceable{failingNoteConvertable{}}, core.On("1"))
	// if got, want := r.Storex(), "?"; got != want {
	// 	t.Errorf("got [%v:%T] want [%v:%T]", got, got, want, want)
	// }
	// r = NewResequencer([]core.Sequenceable{s}, core.On(1))
	// if got, want := r.Storex(), "?"; got != want {
	// 	t.Errorf("got [%v:%T] want [%v:%T]", got, got, want, want)
	// }
}
...
func TestResequence_Replaced(t *testing.T) {
	...
	r := NewResequencer([]core.Sequenceable{s1}, core.On("1"))
	if core.IsIdenticalTo(r, s1) {
		t.Error("should not be identical")
	}
	if !core.IsIdenticalTo(r.Replaced(s1, s2).(Resequencer).Target[0], s2) {
		t.Error("not replaced")
	}
	if !core.IsIdenticalTo(r.Replaced(r, s2), s2) {
		t.Error("should be replaced by s2")
	}
	r = NewResequencer([]core.Sequenceable{r}, core.On("1"))
	if !core.IsIdenticalTo(r.Replaced(r.Target[0], s2).(Resequencer).Target[0], s2) {
		t.Error("not replaced")
	}
	r = NewResequencer([]core.Sequenceable{failingNoteConvertable{}}, core.On("1"))
