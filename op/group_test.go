...
func TestGroup_S(t *testing.T) {
	...
	g := Group{Target: []core.Sequenceable{s}}
...
func TestGroup_Replaced(t *testing.T) {
	...
	g := Group{Target: []core.Sequenceable{s1}}
	if core.IsIdenticalTo(g, s1) {
		t.Error("should not be identical")
	}
	if !core.IsIdenticalTo(g.Replaced(s1, s2).(Group).Target[0], s2) {
		t.Error("not replaced")
	}
	if !core.IsIdenticalTo(g.Replaced(g, s2), s2) {
		t.Error("should be replaced by s2")
	}
	g = Group{Target: []core.Sequenceable{g}}
	if !core.IsIdenticalTo(g.Replaced(g.Target[0], s2).(Group).Target[0], s2) {
		t.Error("not replaced")
	}
	g = Group{Target: []core.Sequenceable{failingNoteConvertable{}}}
